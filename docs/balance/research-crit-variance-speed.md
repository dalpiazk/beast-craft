...
- Speed: target ~2.5–3× slowest:fastest, validated in the simulator. Consider FFX-style heavier-skill gauge costs once skills are authored. **Superseded (user decision, roster retune):** base Speed spans at most 1.15× slowest:fastest (the user's 10–15% expectation; the roster uses 92–105). The 2.5–3× figure had no source (§4) and the simulator did not bear it out; see `docs/design/battle-system.md`, "Starter roster", and `docs/balance/tuning-log.md`. **Amended again (2026-09-23):** turns now grow with sqrt(Speed) (Sword x Staff's ATB model), and the 10–15% target applies to turns rather than to the Speed stat; see `research-sword-x-staff.md`.
