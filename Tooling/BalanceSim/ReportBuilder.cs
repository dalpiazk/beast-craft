...
{
    /// <summary>
    /// Aggregates <see cref="BattleRecord"/>s into the Markdown report. Pure: the output depends
    ...
    public static class ReportBuilder
    {
        ...
        public static string Build(SimOptions options, IReadOnlyList<CreatureSpeciesSO> species, List<BattleRecord> records)
        {
            ...
            report.AppendLine("code and arguments produce a byte-identical report.");
            report.AppendLine();
        ...
        private static void AppendConfiguration(StringBuilder report, SimOptions options, IReadOnlyList<CreatureSpeciesSO> species, int battles, int gamesPerBeast)
        {
            ...
            report.AppendLine("- Levels: " + SimOptions.Join(options.Levels) + "; no gear; no avatar");
            report.AppendLine("- Modes: " + ModeList(options.Modes));
            report.AppendLine("- Arena: " + SimOptions.Arena + ", 1v1, mirrored central start tiles in each deployment zone");
            ...
            report.AppendLine("- Max rounds: " + options.MaxRounds + " (a battle reaching it is a stalemate); base seed: " + options.Seed);
            report.AppendLine("- Standard kit, in fire-priority order:");
            report.AppendLine("  - Blast: " + SimOptions.BlastCategory + ", power " + SimOptions.BlastPower.ToString(CultureInfo.InvariantCulture) +
                              ", range " + SimOptions.BlastRange + ", cooldown " + SimOptions.BlastCooldown);
            report.AppendLine("  - Strike: " + SimOptions.StrikeCategory + ", power " + SimOptions.StrikePower.ToString(CultureInfo.InvariantCulture) +
                              ", range " + SimOptions.StrikeRange + ", cooldown " + SimOptions.StrikeCooldown);
            report.AppendLine("  - Single target, nearest enemy. `elemental` mode: both skills carry the beast's first element. `neutral`");
            report.AppendLine("    mode: both are `None`, isolating stat distribution from the element chart.");
        ...
        {
            report.AppendLine("## Flagged outliers");
            report.AppendLine();
        ...
        private static void AppendStats(StringBuilder report, SimOptions options, IReadOnlyList<CreatureSpeciesSO> species)
        {
            ...
            report.AppendLine();
            report.AppendLine("From `StatCalculator.ComputeStats` with no gear — what each beast actually fought with at the matrix level.");
            report.AppendLine();
            ...
            foreach (CreatureSpeciesSO beast in species)
            {
                ...
                report.AppendLine("| " + beast.DisplayName + " | " + ElementsOf(beast) + " | " + stats.Hp + " | " + stats.Attack + " | " + stats.Defense +
        ...
        private static string ElementsOf(CreatureSpeciesSO beast)
        {
        ...
        private static string ModeList(List<KitMode> modes)
        {
            List<string> names = new List<string>();
