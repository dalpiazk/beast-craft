...
def sprite_entry(m, ims, palette):
    entry = {
        "Name": m["name"],
...
def write_manifest(data, sprites):
    ...
    manifest = {
        "_readme": "GENERATED by Tooling/PixelArt/build.py -- do not edit. Every sprite: its PNG (a horizontal "
                   "strip of Frames frames, each FrameWidth x FrameHeight), frame time, kind and ArtKey (the key "
                   "species and enemies name their art by; an alias entry reuses another sprite's File with a "
                   "Tint); plus the palette (char -> colour) that VFX colours are named from.",
        "SchemaVersion": 1,
        "Palette": palette,
        "Sprites": [sprite_entry(m, ims, palette) for m, ims in sprites],
    }
