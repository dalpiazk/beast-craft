"""Colour lock post-step and QA metrics (character layer only).
...
def reinhard(img, beast, chroma=0.7, light=0.3, mask=None):
    ...
    bm &= blk.mean(-1) > 90                                              # ignore the eye/ink pixels of the block
...
def fidelity(img, beast):
    ...
    blk = np.asarray(Image.open(WORK / f"sketch_{beast}_block.png").convert("RGB"))
    bm = np.any(np.abs(blk.astype(int) - np.array(BG)) > 6, axis=-1) & (blk.mean(-1) > 90)
