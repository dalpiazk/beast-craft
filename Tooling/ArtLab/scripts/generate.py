"""The generation steps: exploration candidates, lock prep, and the design lock (6 seeds).

  python generate.py explore_i2i BEAST [--ipa 0.4] [--strength 0.80] [--cn 0.4] [--cn-end 0.6] [--seed0 400] IDX...
      Candidate sheet inputs: img2img from the randomised colour-mass layouts (layouts.py) + canny-of-layout +
      InstantStyle + the house prompt, then Reinhard to the beast's swatch -> $ARTLAB_WORK/explore/{beast}_{idx}.png
      (generator seed = seed0 + idx). The finals used IDX 0..13.
  python generate.py explore BEAST [--ipa 0.4] SEED...
      Plain txt2img + InstantStyle (fails for non-bird beasts: the style refs are all fire birds; kept for A/B).
  python generate.py prep BEAST PICK.png
      Design-lock inputs from the producer's pick: canny from its own lines (character only), the pick quantised to
      the beast's swatch (the colour block) and the softened block (the img2img init) -> $ARTLAB_WORK/sketch_{beast}_*.
  python generate.py lock BEAST --ipa S [--cn 0.55] [--cn-end 0.7] [--strength 0.80] [--tag lock] [--out DIR] SEED...
      CN img2img from the soft block + canny + InstantStyle + colour negatives, then Reinhard -> {tag}_{beast}_{seed}.png
      (and _prelock.png) in DIR (default $ARTLAB_WORK/seeds), with a log.json of drift and palette distance.

Every call retries a NaN/black output (XPU) up to 4 times on the same seed; if it stays black, run rmblack.py on the
folder and rerun in a fresh process (existing outputs are skipped).
"""
import argparse

from common import *
from colour import reinhard, drift, fidelity
import cv2
...
def explore(beast, ipa, seeds, out=None, tag=None):
    ...
    for s in seeds:
        ...
        f, d = drift(im)
        log_json(out / "log.json", name, dict(beast=beast, seed=s, ipa=ipa, s=round(dt, 1), retries=tries,
...
def explore_i2i(beast, ipa, idxs, strength=0.80, seed0=400, out=None, cn=0.4, cn_end=0.6, tag=None):
    """Design exploration from the randomised colour-mass layouts (layouts.py): img2img at high denoise + canny of
    the layout + InstantStyle + colour/fire negatives, then Reinhard to the beast swatch (as the lock)."""
    out = out or WORK / "explore"
    out.mkdir(parents=True, exist_ok=True)
    ...
    for i in idxs:
        ...
        im, dt, tries = call(pipe, kw, name)
        f, d = drift(im)
        log_json(out / "log.json", name, dict(beast=beast, layout=i, seed=s, ipa=ipa, strength=strength, cn=cn,
                                               cn_end=cn_end, s=round(dt, 1), retries=tries, drift=round(f, 4),
...
def prep(beast, pick):
    ...
    cream BG, (3) the softened block (blur + light gradient + grain, soften.py) as the img2img init."""
...
def lock(beast, ipa, cn, out, seeds, strength=0.80, cn_end=0.7, tag="lock"):
    ...
    for s in seeds:
        ...
        im.save(out / f"{name}.png")
        f, d = drift(im)
        log_json(out / "log.json", name, dict(beast=beast, seed=s, ipa=ipa, cn=cn, strength=strength, s=round(dt, 1),
...
def main():
    ...
    ap.add_argument("cmd", choices=["explore", "explore_i2i", "prep", "lock"])
    ap.add_argument("beast", choices=BEASTS)
    ap.add_argument("args", nargs="*", help="seeds / layout indices, or the pick for prep")
    ap.add_argument("--ipa", type=float, default=0.4, help="InstantStyle scale (lock: 0.6 fire, 0.45 otherwise)")
    ap.add_argument("--cn", type=float, default=None)
    ap.add_argument("--cn-end", type=float, default=None)
    ...
    ap.add_argument("--out")
    a = ap.parse_args()
    nums = [int(x) for x in a.args] if a.cmd != "prep" else []
    if a.cmd == "explore":
        explore(a.beast, a.ipa, nums)
    elif a.cmd == "explore_i2i":
        explore_i2i(a.beast, a.ipa, nums, strength=a.strength, seed0=a.seed0,
                    cn=0.4 if a.cn is None else a.cn, cn_end=0.6 if a.cn_end is None else a.cn_end)
    elif a.cmd == "prep":
        prep(a.beast, a.args[0])
    else:
        lock(a.beast, a.ipa, 0.55 if a.cn is None else a.cn, a.out or WORK / "seeds", nums, strength=a.strength,
             cn_end=0.7 if a.cn_end is None else a.cn_end, tag=a.tag)
