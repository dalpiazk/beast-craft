"""Finishing after the detail pass: SAM box mask (pale areas) -> halo clean -> the bold line pass (house lines).

  python finish.py IN_2x.png OUT.png [--no-tighten] [--peel-paper] [--ground-green Y] [--points "x,y;x,y"]
                   [--halo 28] [line_pass args...]

  --no-tighten     skip tighten() (the phoenix's glow-rim peel); use it for pale or low-chroma beasts (golem, kirin)
  --points         SAM point masks (896-wide px) for pale parts the box mask drops (the golem's standing stones)
  --ground-green Y drop green pixels below y = Y (896-wide px) from the mask (grass tufts beside the feet)
  --peel-paper     peel bare paper from the mask edge, protecting face.FACE and the point masks
...
def add_points(small, m1, pts):
    """SAM 2.1 point prompts for pale parts the box mask drops (standing stones). Per point keep
...
def drop_ground_green(small, m1, y0):
    """Grass tufts beside the feet are ground, not character -> drop green pixels below y0 (1x px) from
...
def peel_paper(rgb, m, u, protect, max_in=40, tol=13.0):
    """SAM's mask (golem) includes a band of bare paper above the moss (the lock's soft glow, repainted as paper
...
def finish(inp, out, band=28, extra=(), tight=True, pts=None, ground=None, peel=False):
...
if __name__ == "__main__":
    ...
    finish(args[0], args[1], band, args[2:], tight, pts, ground, peel)
