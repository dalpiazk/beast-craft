# Beast Craft ArtLab, remaining-seven finals (2026-09-26): the seven rigs (content/art/source/<id>/): inpaint_ok validator, poly_only hand polygons, pivot_override (Tarasque hips).
...
Outputs in $ARTLAB_OUT/rig/ (as run: ai-art-final/<beast>/rig/): character.png (cropped, pivot = bottom centre), character_full.png (canvas),
parts/*.png, parts.json, parts_overlay.png, parts_recomposed.png.
"""
import sys
from common import *
import cv2
...
def load_golem_parts():
    p = pathlib.Path(__file__).with_name("golem_parts.json")
    if p.exists():
...
def load_parts_json(beast):
    """New-beast finals: {beast}_parts.json next to this script, coordinates in 1x (896-wide) px -> x2 to final px."""
    p = pathlib.Path(__file__).with_name(f"{beast}_parts.json")
    d = json.loads(p.read_text())
    for k, v in d["parts"].items():
        for kk in ("pos", "neg", "clip"):
            if kk in v:
                v[kk] = [(2 * x, 2 * y) for x, y in v[kk]]
        if "box" in v:
            v["box"] = tuple(2 * c for c in v["box"])
    PARTS[beast] = d
...
def sam_part(img_small, s, spec, fg_small):
    ...
    if spec.get("poly_only"):                      # new-beast finals: small busy parts (thunderbird head) -> hand polygon
        return clip & fg_small, 0.0, "polygon"
...
def inpaint_pipe():
    ...
    ip.set_progress_bar_config(disable=True); ip.vae.disable_tiling()
    return ip
...
def inpaint_ok(r, m):
    """New-beast finals: this session the XPU inpaint sometimes returns a flat grey fill or colour-bar garbage that
    passes the black check. Reject if the inpainted area is near-flat (std < 3) or > 2% of it is saturated and
    > 30 deg from every chromatic swatch hue."""
    if m.sum() < 50:
        return True
    px = r[m].astype(np.float32)
    if px.std(0).mean() < 3:
        print("inpaint rejected: flat", flush=True); return False
    ys, xs = np.nonzero(m)
    g = cv2.cvtColor(r, cv2.COLOR_RGB2GRAY).astype(np.float32)[ys.min():ys.max() + 1, xs.min():xs.max() + 1]
    mm = m[ys.min():ys.max() + 1, xs.min():xs.max() + 1]
    if g.shape[0] > 8 and g.shape[1] > 8:
        dy = np.abs(np.diff(g, axis=0))[:, :-1][mm[1:, 1:]]; dx = np.abs(np.diff(g, axis=1))[:-1][mm[1:, 1:]]
        ratio = dy.mean() / (dx.mean() + 1e-3)
        if ratio > 1.35:
            print(f"inpaint rejected: streaks {ratio:.2f}", flush=True); return False
    b = _CUR.get("beast")
    if b in SWATCH:
        hsv = cv2.cvtColor(r, cv2.COLOR_RGB2HSV_FULL).astype(np.float32)[m]
        hue, sat = hsv[:, 0] * 360 / 256, hsv[:, 1] / 255
        sw = np.array([[int(h[i:i + 2], 16) for i in (0, 2, 4)] for h, _ in SWATCH[b]], np.uint8)[None]
        sh = cv2.cvtColor(sw, cv2.COLOR_RGB2HSV_FULL).astype(np.float32)[0]
        hues = sh[sh[:, 1] / 255 > 0.15, 0] * 360 / 256
        d = np.min(np.abs(((hue[:, None] - hues[None]) + 180) % 360 - 180), -1)
        off = float(((d > 30) & (sat > 0.45)).mean())
        if off > 0.02:
            print(f"inpaint rejected: off-palette {off:.3f}", flush=True); return False
    return True
...
def sd_inpaint(ip, rgb, mask, prompt, seed=7):
    ...
    for attempt in range(6):
        ...
        r = np.asarray(res)
        if r.max() > 8 and np.isfinite(r).all() and inpaint_ok(r, np.asarray(mi) > 127):
            break
...
def main(beast, do_inpaint=True):
    ...
    if beast == "golem":
        ...
        load_parts_json(beast)
    cfg = PARTS[beast]
    OD = FINAL / "rig"; (OD / "parts").mkdir(parents=True, exist_ok=True)
    rgb = np.asarray(Image.open(FINAL / f"{beast}_final.png").convert("RGB"))   # $ARTLAB_OUT/<beast>_final.png
    H2, W2 = rgb.shape[:2]
    extra = None
    ...
    for i, k in enumerate(ORDER):
        ...
        if region.sum() > 400:
            ...
            if ip is not None:
                ...
                hue = hsv[..., 0] * 360 / 256
                lo = {"golem": 150, "leviathan": 331, "thunderbird": 331}.get(beast, 75)   # teal/blue are on-palette for Water/Lightning -> cool guard off
    ...
    for k in ORDER:
        ...
        if "pivot" in cfg["parts"][k]:              # pivot_override (hip/shoulder set by hand, 1x coords)
            piv = tuple(int(2 * v) for v in cfg["parts"][k]["pivot"])
        elif PARENT[k] is None:
            piv = (fx, fy)
        else:
