# Beast Craft ArtLab, remaining-seven finals (2026-09-26): the SAM line-pass character mask for the seven finals.
...
def line_mask(rgb_u8):
    ...
    ys, xs = np.nonzero(a)
    if len(xs) < 50 or a.mean() > 0.6:                 # candidates: busy/scenic ground -> generous frame box
        h, w = a.shape
        xs, ys = np.array([int(w * .04), int(w * .96)]), np.array([int(h * .03), int(h * .97)])
    pad = int(12 * u)
