# Beast Craft ArtLab, remaining-seven finals (2026-09-26): the face-masked detail pass (body 2x, face 1x) of the seven finals, run by chain.sh.
...
--tile-cn  (round 3) tile-ControlNet weight (xinsir/controlnet-tile-sdxl-1.0, control = the tile itself). Lets the
...
DETAIL_PROMPT = ("no humans, original creature, cute, painterly, watercolor texture, visible brush strokes, soft gradient shading, "
                 "soft cel shading, warm dappled light, rim light, fine texture, clean lineart, masterpiece, high score, absurdres")
    ...
    for y in ys:
        for x in xs:
            ...
            if mask is not None and mask[y:y + th_, x:x + tw_].max() < 0.05:
                ...
                with Timer() as t:
                    for attempt in range(5):          # final: XPU NaN/black tiles -> retry the same tile
